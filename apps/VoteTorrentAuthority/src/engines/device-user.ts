/**
 * device-user.ts — Generate-on-first-run secp256k1 device identity (D-02 / Phase 16).
 *
 ...
 *
 * AUTH-01 compliance: keys are hex-encoded via `bytesToHex` from @noble/curves/abstract/utils.
 * Do NOT call the native Uint8Array serializer — that produces a comma-separated decimal string,
 * not a valid secp256k1 hex key.
 *
 * Security note (T-16-01): the private key is stored in plaintext AsyncStorage.
 * Android Keystore migration is a deferred v1.3 hardening task.
 */
...
export async function getOrCreateDeviceUser(displayName: string): Promise<User> {
	...
	// Generate a real secp256k1 keypair (CSPRNG — T-16-02 mitigation)
 ...
 *
 * Used by the election signing pipeline (Plan 03) to produce real secp256k1 signatures.
 */
