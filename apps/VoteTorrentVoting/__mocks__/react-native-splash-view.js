 ...
 * `VotingAppProvider.tsx` started importing `hideSplash` from this package (Phase 44-07's real
 * composition-root swap), every test that transitively requires that module — even ones that
 * never render `<VotingAppProvider>` (e.g. via `jest.requireActual` in a partial mock, or an
 * unrelated screen that imports `useVotingApp`) — crashed at require-time. Mirrors the
