/**
 * Behavioral tests for VotingAppProvider — the real composition root (Phase 44-07, D-02/D-04/D-07).
 *
 * Proves the plan's `must_haves`:
 *   - Rendering VotingAppProvider (wrapped in a mocked CadreNodeProvider, mirroring App.tsx's
 *     nesting) flips `isInitialized` to true after the mocked D-07 seed + network open resolve,
 *     and calls `hideSplash`.
 *   - `useVotingApp()` surfaces a real `getEngine` accessor plus `seededElectionId`/`sign`
 ...
 * App.test.tsx / this app's own App.test.tsx convention) — this test proves VotingAppProvider's
...
import {VotingAppProvider, useVotingApp} from '../VotingAppProvider';
import type {VotingAppContextType} from '../types';
...
describe('VotingAppProvider — real composition root (D-02/D-04/D-07)', () => {
	...
	it('useVotingApp() exposes a real getEngine accessor plus seededElectionId + sign (not the removed isRegistered mock boolean)', async () => {
		mockSeedDevNetwork.mockImplementation(seedRealNetwork);
		const {captured} = renderProvider();
		await flushBoot();
