...
export function CadreNodeProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    async function bootNode() {
      try {
        // Peer-identity store — 'votetorrent-voting-cadre-node' gives a stable
        ...
        const identityDb = openOptimysticRNDb({
          ...
          name: 'votetorrent-voting-cadre-node',
        });
        ...
        const scopedStorageProvider = createScopedRnStorageProvider('votetorrent-voting-strand');
