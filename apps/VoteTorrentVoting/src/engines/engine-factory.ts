 ...
 *
 * Lifecycle:
 *   - One EngineFactory instance per VotingAppProvider (useRef, app-lifetime).
 ...
 * getEstablishedContext result never returned to VotingAppProvider/screens.
...
export class EngineFactory {
	...
	/** Called by VotingAppProvider after resolving the device user, before getEngine("network"). */
	 ...
	 * Set by VotingAppProvider in the same useEffect that registers setGetPeerCount.
	 */
	private node: StrandHost | null = null

	/** Called by VotingAppProvider when the CadreNode boots (mirrors setGetPeerCount / D-04). */
	...
	/** Expose the shared NetworksEngine for initialize() in VotingAppProvider. */
	 ...
	 * Must be called by VotingAppProvider on network switch or "Start Fresh" (not by
	...
	private cacheKey(engineName: string, initParams?: unknown): string {
		...
		// Screens call getEngine("network") with no params while VotingAppProvider
	...
	private async buildEngine(engineName: string, initParams?: unknown): Promise<unknown> {
		switch (engineName) {
			case 'network': {
				...
				//
				// Screens call getEngine("network") with NO params; VotingAppProvider
