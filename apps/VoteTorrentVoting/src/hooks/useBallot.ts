 ...
 * live = false; };`.
 *
 * Takes `getBallot` as a PARAMETER rather than calling `useVotingApp()` internally, so each
 * screen still makes its own literal `useVotingApp()` call — preserving SHELL-03/D-06's
 * source-scan-enforced "every screen calls useVotingApp(" convention
 * (`__tests__/no-inline-mock-imports.test.ts`). A screen-invisible `useVotingApp()` call buried
