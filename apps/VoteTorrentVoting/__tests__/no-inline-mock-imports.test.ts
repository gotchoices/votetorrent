 ...
 * through useVotingApp(), never a direct providers/mockData import" rule. This is the automated
...
describe('no inline mockData imports in screens (SHELL-03, D-06)', () => {
	...
	test('every screen file calls useVotingApp(', () => {
		const offenders: string[] = [];
		for (const file of screenFiles) {
			const text = stripComments(fs.readFileSync(file, 'utf8'));
			if (!/\buseVotingApp\(/.test(text)) {
				offenders.push(path.relative(screensDir, file));
			}
		}
