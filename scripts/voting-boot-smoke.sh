#!/usr/bin/env bash
#
# voting-boot-smoke.sh
#
# Purpose  : On-device cold-start boot smoke for apps/VoteTorrentVoting. Proves the
...
#            VotingAppProvider.tsx once the real dev-seed + network open completes, while
...
#
# Usage    : ./scripts/voting-boot-smoke.sh
#
# Prerequisites:
#   - adb in PATH (Android SDK Platform Tools); one AVD/device connected.
#   - Metro dev server for apps/VoteTorrentVoting reachable (script starts one if absent).
...
APP_DIR="apps/VoteTorrentVoting"
GRADLE_BUILD="$APP_DIR/android/app/build.gradle"

# --- Scratchpad for captures (override with VOTING_SMOKE_OUT); portable default.
OUT_DIR="${VOTING_SMOKE_OUT:-${TMPDIR:-/tmp}/voting-boot-smoke}"
mkdir -p "$OUT_DIR"
LOGCAT_CAP="$OUT_DIR/voting-boot-logcat.txt"
SCREENSHOT="$OUT_DIR/voting-boot-home.png"

BOOT_TOKEN='[voting-boot] VotingAppProvider isInitialized'
# Every known bundle/boot failure signal from 44-UAT.md. Any hit = FAIL.
FAILURE_SIGNALS=(
  'The development server returned response error code: 500'
  '__extends'
  'runtime not ready'
  'VoteTorrentVoting has not been registered'
...
log() { printf '[voting-boot-smoke] %s\n' "$*"; }
