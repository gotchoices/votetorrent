...
export function calculateCompositePublicKey(stateView: Readonly<DKGStateView>): Uint8Array | null {
    ...
    if (Object.keys(stateView.participantContributions).length < stateView.terms.n) {
         console.error("Cannot calculate public key: Not all participants have posted contributions.");
         return null;
    }

    for (const id of stateView.terms.participantIds) {
        ...
        if (!commData || !commData.commitments || commData.commitments.length === 0) {
            console.error(`Cannot calculate public key: Commitments missing for ${id}.`);
            // This indicates a failure in Phase 1 completion check.
            return null;
        }
        try {
