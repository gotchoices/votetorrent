...
import type { DKGTerms } from './dkg-terms.js';
...
import { evaluatePolynomial, bigintToBytes, ensureValidPrivateKey } from './helpers.js';
 ...
 * @param terms The DKG terms.
 * @param termsHash The hash of the terms.
 * @returns Initialized private state.
 * @throws Error if keyholderId is not found in terms.
 */
export function initializeKeyholderPrivateState(keyholderId: string, terms: Readonly<DKGTerms>, termsHash: string): KeyholderPrivateState {
    const index = terms.participantIds.indexOf(keyholderId);
    if (index === -1) {
 ...
 * @param terms The DKG terms.
 * @param termsHash The hash of the DKG terms.
...
export function determineKeyholderAction(
    ...
    // Basic checks
    if (stateView.termsHash !== termsHash) {
        // This indicates a fundamental mismatch, but the caller should handle it.
        // We don't have private state here to mark an error.
        return { type: DKGActionType.NoAction, reason: "Terms hash mismatch" };
    }
    if (!terms.participantIds.includes(keyholderId)) {
    ...
    const allCommitmentsPosted = terms.participantIds.every(id => !!stateView.participantContributions[id]?.commitments);
    ...
    const allSharesPosted = terms.participantIds.every(id =>
    ...
    try {
        ...
        for (let k = 0; k < t; k++) {
            ...
            if (!commitment_Cik_bytes) {
                ...
                return false; // Missing commitment data
            }
...
export function computeKeyholderUpdate(
    ...
    if (stateView.termsHash !== nextPrivateState.termsHash) {
        nextPrivateState.error = "Term hash mismatch";
        return { publicUpdate: null, nextPrivateState: null }; // Fatal error
    }
    if (!terms.participantIds.includes(nextPrivateState.id)) {
        nextPrivateState.error = `Keyholder ${nextPrivateState.id} not found in terms`;
         return { publicUpdate: null, nextPrivateState: null }; // Fatal error
    }

    const determinedAction = action ?? determineKeyholderAction(nextPrivateState.id, terms, nextPrivateState.termsHash, stateView);
    ...
    // --- Perform Computation based on Action ---
    try {
        switch (determinedAction.type) {
            case DKGActionType.PostCommitments: {
                 if (nextPrivateState.polynomial) {
                     // Avoid re-generating if called unnecessarily
                     // This might indicate a logic error in the caller or state progression
                     return { publicUpdate: null, nextPrivateState };
                 }

                    const poly = [];
                 for (let k = 0; k < terms.threshold; k++) {
                    // Generate a random private key and convert it directly to a valid scalar bigint
                    const coeffBytes = utils.randomPrivateKey();
                    // Use normPrivateKeyToScalar for validation and conversion
                    const coeff = utils.normPrivateKeyToScalar(coeffBytes);
                    poly.push(coeff);
                 }
                 nextPrivateState.polynomial = poly; // Store private polynomial in next state
                    ...
                    const payload: PostCommitmentsPayload = {
                        ...
                        idNum: nextPrivateState.idNum, // Use derived idNum
            ...
            case DKGActionType.PostShares: {
                if (!nextPrivateState.polynomial) {
                     // Should not happen if determineAction is correct and state flows properly
                     nextPrivateState.error = "Polynomial not available for share calculation";
                     return { publicUpdate: null, nextPrivateState: null }; // Fatal state inconsistency
                }

                // Check if already computed/posted in stateView
                if (stateView.participantContributions[nextPrivateState.id]?.distributedShares?.length === terms.n) {
                    return { publicUpdate: null, nextPrivateState }; // Already done
                }

                const shares: EncryptedShare[] = [];
                for (let i = 0; i < terms.participantIds.length; i++) {
                     const targetId = terms.participantIds[i]!;
                     const targetIdNum = BigInt(i + 1); // Use 1-based index for evaluation
                     const shareValue = evaluatePolynomial(nextPrivateState.polynomial, targetIdNum);
                     // TODO: Actual Encryption would happen here using target's public key
                     // For now, we store the raw share value.
                     shares.push({
                         sourceId: nextPrivateState.id,
                         targetId: targetId,
                         encryptedShare: shareValue // Placeholder for actual encryption
                     });
                }

                // ** Optional: Securely discard polynomial *after* this point **
                // nextPrivateState.polynomial = null; // Uncomment if desired
            ...
            case DKGActionType.VerifyShares: {
                if (nextPrivateState.verifiedReceivedShares) {
                    // Already verified, no new public update needed.
                    return { publicUpdate: null, nextPrivateState };
                }

                const allCommitments = stateView.participantContributions;
                const allDistributedShares: EncryptedShare[] = terms.participantIds.flatMap(pid =>
                ...
                if (sharesForMe.length !== terms.n) {
                    // Not all shares received yet, cannot verify. determineAction should prevent this.
                ...
                for (const receivedShare of sharesForMe) {
                    ...
                    const isValid = verifyReceivedShare(nextPrivateState.idNum, receivedShare, senderCommitments);
                    if (!isValid) {
                        allValid = false;
                        failureReason = `Invalid share received from sender ${receivedShare.sourceId}`;
                        break; // Stop verification on first failure
                    } else {
                         // TODO: Decrypt share here before storing if it was actually encrypted
                ...
                if (allValid) {
                     nextPrivateState.verifiedReceivedShares = verifiedMap; // Store verified shares
                     // Compute final share immediately after verification
                     nextPrivateState.computedFinalShare = computeFinalShare(
                         nextPrivateState.id,
                         nextPrivateState.idNum,
                         nextPrivateState.verifiedReceivedShares // Pass the newly set map
                     );
                } else {
                    // Verification failed, ensure no final share is computed or stored
                    nextPrivateState.verifiedReceivedShares = null;
                    nextPrivateState.computedFinalShare = null;
                    // Mark a non-fatal error/status? The public update already reports failure.
                    // We don't set nextPrivateState.error here as the process might succeed overall
                    // if enough others succeed. The public verification state handles this.
                }
        ...
        nextPrivateState.error = `Computation error during ${determinedAction.type}: ${error?.message ?? error}`;
        return { publicUpdate: null, nextPrivateState: null }; // Fatal error
    }
}
